...
import pe.hgs.truler.phase.analysis.FinalResult;
import pe.hgs.truler.phase.analysis.JointSetting;
import pe.hgs.truler.phase.analysis.ProfileSelection;
import pe.hgs.truler.phase.PhaseSignal;
import pe.hgs.truler.phase.analysis.SubAssessment;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Intent itPS = new Intent(this, ProfileSelection.class);
        startActivityForResult(itPS, PhaseSignal.PHASE_ANL_PROFILE_SELECTING);
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            case PhaseSignal.PHASE_ANL_PROFILE_SELECTING:										//프로필 작성 화면 종료 시
				//*
				Intent itIS = new Intent(Intent.ACTION_PICK);
				itIS.setType("image/*");
				startActivityForResult(itIS, PhaseSignal.PHASE_ANL_IMAGE_LOADING);
				//*/
                break;

			case PhaseSignal.PHASE_ANL_CAMERA:													//카메라 촬영 완료 시
			case PhaseSignal.PHASE_ANL_IMAGE_LOADING:											//이미지 선택 완료 시
				Intent itJS;
				if(resultCode == -1) {
					...
					startActivityForResult(itJS, PhaseSignal.PHASE_ANL_JOINT);
				} else {		//주로 0 아무것도 선택 안했을 시 처리
					Logger.warn("No image selected");
					itJS = new Intent(this, ProfileSelection.class);
					startActivityForResult(itJS, PhaseSignal.PHASE_ANL_PROFILE_SELECTING);
				}
				break;
            case PhaseSignal.PHASE_ANL_JOINT:													//관절 선택 완료 시
				switch (resultCode) {
					case RESULT_OK:
				...
				startSubAssessment();
				break;
			case PhaseSignal.PHASE_ANL_SUB_ASSESSMENT:											//평가 완료 시
				switch (resultCode) {
					case RESULT_OK:
						if(data.getBooleanExtra("Assessment Result", false)) {
							...
							startActivityForResult(itResult, PhaseSignal.PHASE_ANL_RESULT);
						} else {
							itJS = new Intent(this, JointSetting.class);
	...
	private void startSubAssessment() {
		...
		startActivityForResult(itAssessment, PhaseSignal.PHASE_ANL_SUB_ASSESSMENT);
	}
